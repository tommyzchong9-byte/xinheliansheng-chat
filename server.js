...
<div id="app">
  <div id="sidebar">
    ...
    <div class="user-box">
      ...
      <button id="logoutBtn" title="切换用户" style="background:none;border:none;cursor:pointer;font-size:16px;padding:2px;opacity:0.5;line-height:1;">⇄</button>
    </div>
  </div>
...
// langCodes not needed, server handles translation
...
if(savedName){
  ...
  socket.emit('join', {username: savedName, room: savedRoom});
} else {
...
document.getElementById('logoutBtn').addEventListener('click',()=>{
  ...
  localStorage.removeItem('xlsUsername');
  localStorage.removeItem('xlsRoom');
  location.reload();
});
...
function switchRoom(room){
  ...
  currentRoom=room; localStorage.setItem('xlsRoom',room); socket.emit('switchRoom',{room});
...
async function doTranslate(text, langLabel, boxId, btn){
  ...
  if(srcLang===targetLang||(srcLang==='zh'&&targetLang==='zh')){
...
const SW = 'const CACHE="xlsv5";self.addEventListener("install",e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll([" /"]))));self.addEventListener("fetch",e=>{if(e.request.url.includes("/socket.io/")||e.request.method==="POST")return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request)));});';
...
app.post('/translate', express.json(), async (req, res) => {
  ...
  const tgtCode = langMap[target];
  if(!tgtCode) return res.json({error:'unknown language'});
  try {
    const https = require('https');
    const url = 'https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl='+tgtCode+'&dt=t&q='+encodeURIComponent(text);
    ...
    res.json({error:'translation failed: '+e.message});
  }
});
