...
const SW = 'const CACHE="xlsv6";self.addEventListener("install",e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll([" /"]))));self.addEventListener("fetch",e=>{if(e.request.url.includes("/socket.io/")||e.request.method==="POST")return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request)));});';
