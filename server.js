...
const langCodes={'中':'zh','英':'en','日':'ja'};
...
const SW = 'const CACHE="xlsv3";self.addEventListener("install",e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll([" /"]))));self.addEventListener("fetch",e=>{if(e.request.url.includes("/socket.io/"))return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request)));});';
