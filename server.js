...
socket.on('onlineUsers',users=>{
  ...
  users.forEach(u=>{
    ...
    el.innerHTML='<div class="av" style="width:22px;height:22px;font-size:9px;background:'+u.color+'">'+u.name[0].toUpperCase()+'</div><span class="on2">'+u.name+'</span><span style="font-size:10px;color:var(--muted)">#'+u.room+'</span><div class="dg"></div>';
    list.appendChild(el);
  });
});
...
function renderMsg(msg){
  ...
  meta.innerHTML='<span class="mu" style="color:'+msg.color+'">'+esc(msg.username)+'</span><span class="mt">'+ft(msg.timestamp)+'</span>';
  g.appendChild(meta);
...
const SW = 'const CACHE="xlsv7";self.addEventListener("install",e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll([" /"]))));self.addEventListener("fetch",e=>{if(e.request.url.includes("/socket.io/")||e.request.method==="POST")return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request)));});';
