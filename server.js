...
async function uploadAvatar(file){
  ...
  fd.append('transformation','w_200,h_200,c_fill,g_face,r_max');
  try{
...
loginAvInput.addEventListener('change',async()=>{
  ...
  if(url){myAvatarUrl=url;localStorage.setItem('xlsAvatar',url);}
});
...
socket.on('userConfirmed',({username,color})=>{
  ...
  const av=document.getElementById('myAvatar');
  if(myAvatarUrl){
    av.innerHTML='<img src="'+myAvatarUrl+'" style="width:28px;height:28px;border-radius:50%;object-fit:cover;"/>';
    socket.emit('updateAvatar',{url:myAvatarUrl});
  } else {
