...
function getRoomUsers(room) {
  const u = []; onlineUsers.forEach((v,k) => { if(v.room===room) u.push({name:k,color:v.color}); }); return u;
}
function getAllUsers() {
  const u = []; onlineUsers.forEach((v,k) => u.push({name:k,color:v.color,room:v.room})); return u;
}
...
io.on('connection', (socket) => {
  ...
  socket.on('sendMessage', ({text, room}) => {
    if (!me || !text?.trim() || !rooms[room]) return;
